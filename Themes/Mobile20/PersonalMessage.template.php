...
/***********************************************************************************
*                                                                                 *
* SMF Mobile Theme v1.0.3                                                      	  *
* Copyright (c) 2016 by SMFMobileTheme.com All rights reserved.       			  *
...
<div data-role="header" style="overflow:hidden;" data-position="fixed">
	...
	<div data-role="navbar">
		<ul>
			<li><a href="', $scripturl, '?action=pm;f=inbox"', (isset($_GET['f']) && $_GET['f'] == 'inbox' ? ' class="ui-btn-active"' : ''), '>', $txt['inbox'],'</a></li>
			<li><a href="', $scripturl, '?action=pm;f=sent"', (isset($_GET['f']) && $_GET['f'] == 'sent' ? ' class="ui-btn-active"' : ''), '>', $txt['sent_items'],'</a></li>
			<li><a href="', $scripturl, '?action=pm;sa=send"', (isset($_GET['sa']) && in_array($_GET['sa'], array('send', 'send2')) ? ' class="ui-btn-active"' : ''), '>', $txt['new_message'],'</a></li>
			<li><a href="', $scripturl, '?action=pm;sa=search"', (isset($_GET['sa']) && in_array($_GET['sa'], array('search', 'search2')) ? ' class="ui-btn-active"' : ''), '>', $txt['search'],'</a></li>
		</ul>
	</div><!-- /navbar -->
</div><!-- /header -->
		...
		if ($context['get_pmessage']('message', true))
		{
			...
			while ($message = $context['get_pmessage']('message'))
			{
				...
				<li data-role="list-divider">
					', $txt['by'], ' ', $message['member']['name'], ' ', $txt['on'], ' ', $message['time'], '
					<a href="#popupMenuMsg', $message['id'], '" data-rel="popup" data-transition="slideup" class="ui-btn ui-btn-inline ui-icon-bars ui-btn-icon-left" data-iconpos="notext" border="0" style="float: right;"></a>
				</li>
				<li>
					<div class="inner">
						', $message['body'], '
...
if (!empty($context['page_info']['num_pages']))		
{
	for ($i = 0; $i < $context['page_info']['num_pages']; $i++)
	{
		...
		echo'
				<option value="', $scripturl,'?action=pm;f=' . $context['folder'] . (isset($_REQUEST['l']) ? ';l=' . (int) $_REQUEST['l'] : '') . ';sort=' . $context['sort_by'] . ($descending ? ';desc' : '') . ';start=', $page,'"', (($i + 1) == $context['page_info']['current_page'] ? ' selected="selected"' : ''), '>', ($i + 1), '</option>';
	}
}
else
	echo'
...
echo '
	...
	while ($message = $context['get_pmessage']('subject'))
	{
		echo '
			<li>
				<a href="', $scripturl . '?action=pm;pmid=' . $message['id'] . ';kstart;f=' . $context['folder'] . ';start=' . $context['start'] . ';sort=' . $context['sort_by'] . ($context['sort_direction'] == 'up' ? ';' : ';desc') . '#msg', $message['id'], '">
					<h2>', $message['subject'], ' ', $message['is_unread'] ? '&nbsp;<img src="' . $settings['lang_images_url'] . '/new.gif" alt="' . $txt['new'] . '" />' : '', '</h2>
					<p>', $message['time'], '</p>
					<p>', ($context['from_or_to'] == 'from' ? $message['member']['name'] : (empty($message['recipients']['to']) ? '' : strip_tags(implode(', ', $message['recipients']['to'])))), '</p>
				</a>
			</li>';
	}
...
function template_send()
{
	...
	echo '
					<div id="pm_to" class="clear_right">
	...
	echo '
					...
					<div class="clear_left">
						<span', (isset($context['post_error']['no_message']) ? ' class="error"' : ''), '>', $txt['message'], ':</span>
					</div>';
	...
	echo '
				<label for="outbox">', $txt['pm_save_outbox'], ':</label>
	            <select name="outbox" id="outbox" data-role="slider" tabindex="', $context['tabindex']++, '">
	                <option value="0">Off</option>
	                <option value="1"', $context['copy_to_outbox'] ? ' selected="selected"' : '', '>On</option>
	            </select>
