...
function get_config_func()
{
    ...
    foreach($mobiquo_config as $key => $value)
    {
        ...
        }else if (!in_array($key, array('is_open', 'guest_okay', 'mod_function', 'conflict_mod', 'push')))
        {
    ...
    if ($isTTServerCall){
        ...
        $config_list['release_timestamp'] = new xmlrpcval(1463490207, 'string');;
        $TT_bannerControlData = isset($modSettings['tt_banner_control']) ? @unserialize($modSettings['tt_banner_control']) : false;
        $config_list['smartbanner_info'] = new xmlrpcval(json_encode($TT_bannerControlData), 'string');;
        $push_slug = @unserialize(base64_decode($modSettings['push_slug']));
        $config_list['push_slug'] = new xmlrpcval(json_encode($push_slug), 'string');;
...
function login_func()
{
    ...
    $user_type = tt_get_user_type($user_info['id']);
...
function get_topic_func()
{
    ...
    foreach($context['topics'] as $topic) {
        ...
        $xmlrpc_topic = new xmlrpcval(array(
            ...
            'topic_author_user_type' => new xmlrpcval(tt_get_user_type($topic['first_post']['member']['id']), 'string'),
    ...
    $response = new xmlrpcval(
        array(
            'total_topic_num' => new xmlrpcval($mode == 'TOP' ? $board_info['sticky_num'] : $context['real_topic_count'], 'int'),
...
function get_thread_func()
{
    ...
    while ($message = get_post_detail()) {
        ...
        $xmlrpc_post = array(
            ...
            'post_content'      => new xmlrpcval($mesageBody, 'base64'),
            'post_author_id'    => new xmlrpcval($message['member']['id']),
            'post_author_name'  => new xmlrpcval(basic_clean($message['member']['name']), 'base64'),
            'post_author_user_type'  => new xmlrpcval(tt_get_user_type($message['member']['id']), 'string'),
...
//            'can_unapprove'     => new xmlrpcval($message['can_unapprove'] ? true : false, 'boolean'),
    ...
    $response = array(
        ...
        'topic_author_id'=> new xmlrpcval(isset($topicAuthor['id_member']) ? $topicAuthor['id_member'] : 0, 'string'),
        'topic_author_name'=> new xmlrpcval(isset($topicAuthor['real_name']) ? $topicAuthor['real_name'] : '', 'base64'),
        'topic_author_avatar'=> new xmlrpcval(isset($topicAuthor['avatar']) ? $topicAuthor['avatar'] : 0, 'string'),
        'topic_author_user_type' => new xmlrpcval(tt_get_user_type(isset($topicAuthor['id_member']) ? $topicAuthor['id_member'] : 0), 'string'),
...
function get_user_info_func()
{
    ...
    $is_ignored = in_array($context['member']['id'], $user_info['ignoreusers']);
...
function get_user_topic_func()
{
 ...
 'last_reply_author_name'       => new xmlrpcval(basic_clean($topic_info['last_display_name']), 'base64'),
            'short_content'     => new xmlrpcval(basic_clean($topic_info['first_body'], 200, 1), 'base64'),
...
function get_unread_topic_func()
{
    ...
    foreach($context['topics'] as $topic)
    {
        ...
        $xmlrpc_topic = new xmlrpcval(array(
            ...
            'topic_id'          => new xmlrpcval($topic['id'], 'string'),
            'topic_title'       => new xmlrpcval(basic_clean($topic['subject']), 'base64'),
            'post_author_id'  => new xmlrpcval(basic_clean($topic['last_post']['member']['id']), 'string'),
            'post_author_name'  => new xmlrpcval(basic_clean($topic['last_post']['member']['name']), 'base64'),
            'post_author_user_type'  => new xmlrpcval(tt_get_user_type($topic['last_post']['member']['id']), 'string'),
            'post_time'         => new xmlrpcval($topic['last_post']['time'],'dateTime.iso8601'),
            'timestamp'         => new xmlrpcval($topic['last_post']['timestamp'], 'string'),
            'reply_number'      => new xmlrpcval($topic['replies'], 'int'),
            'view_number'       => new xmlrpcval($topic['views'], 'int'),
            'short_content'     => new xmlrpcval(basic_clean($topic_info['last_body'], 200, 1), 'base64'),
            'icon_url'          => new xmlrpcval($topic_info['last_poster_avatar']),
...
function get_new_topic_func()
{
    ...
    foreach ($context['posts'] as $topic)
    {
        $xmlrpc_topic = new xmlrpcval(array(
            ...
            'reply_number'      => new xmlrpcval($topic['replies'], 'int'),
            'view_number'       => new xmlrpcval($topic['views'], 'int'),
            'short_content'     => new xmlrpcval(basic_clean($topic['preview'], 200, 1), 'base64'),
            'post_author_id'    => new xmlrpcval($topic['poster']['id']),
            'post_author_name'  => new xmlrpcval(basic_clean($topic['poster']['name']), 'base64'),
            'post_author_user_type'  => new xmlrpcval(tt_get_user_type($topic['poster']['id']), 'string'),
...
function get_latest_topic_func()
{
    ...
    foreach ($context['posts'] as $topic)
    {
        $xmlrpc_topic = new xmlrpcval(array(
            ...
            'reply_number'      => new xmlrpcval($topic['replies'], 'int'),
            'view_number'       => new xmlrpcval($topic['views'], 'int'),
            'short_content'     => new xmlrpcval(basic_clean($topic['preview'], 200, 1, FALSE), 'base64'),
            'post_author_id'    => new xmlrpcval($topic['poster']['id']),
            'post_author_name'  => new xmlrpcval(basic_clean($topic['poster']['name']), 'base64'),
            'post_author_user_type'  => new xmlrpcval(tt_get_user_type($topic['poster']['id']), 'string'),
            'new_post'          => new xmlrpcval($topic['is_new'], 'boolean'),
   ...
   // $total_topic_num = min(100, $modSettings['totalTopics']);
    $total_topic_num = $context['topic_num'];
...
function get_participated_topic_func()
{
    ...
    foreach ($context['topics'] as $topic)
    {
        ...
        $xmlrpc_topic = new xmlrpcval(array(
            ...
            'post_author_id'  => new xmlrpcval(basic_clean($topic_info['last_id_member']), 'string'),
            'post_author_user_type'  => new xmlrpcval(tt_get_user_type($topic_info['last_id_member']), 'string'),
            'new_post'          => new xmlrpcval($topic_info['new'] ? true : false, 'boolean'),
...
function create_message_func()
{
    ...
    $xmlrpc_create_message = new xmlrpcval(array(
        'result'    => new xmlrpcval($id_pm ? true : false, 'boolean'),
        'msg_id'     => new xmlrpcval($id_pm),
    ), 'struct');

    return new xmlrpcresp($xmlrpc_create_message);
}
...
function get_box_func()
{
    ...
    foreach ($context['messages'] as $pm)
    {
        ...
        $pm_list[] = new xmlrpcval(array(
            ...
            'msg_from_user_type'   => new xmlrpcval(tt_get_user_type($pm['msg_from_id'])),
            ...
            'short_content' => new xmlrpcval(basic_clean($pm['body'], 200, 1), 'base64'),
    ...
    if (isset($context['get_topics']))
    {
        while ($topic = $context['get_topics']())
        {
            ...
            $xmlrpc_topic = new xmlrpcval(array(
                ...
                'post_author_name'       => new xmlrpcval(basic_clean($topic['matches'][0]['member']['name']), 'base64'),
                 'post_author_user_type'       => new xmlrpcval(tt_get_user_type($topic['matches'][0]['member']['id']), 'string'),
        ...
        foreach($context['exttMbqRecords'] as $topic)
        {
           ...
           'post_author_name'       => new xmlrpcval(basic_clean($member['member_name']), 'base64'),
                'short_content'     => new xmlrpcval(basic_clean($topic['body'], 200, 1), 'base64'),
                'icon_url'          => new xmlrpcval($topic['matches'][0]['member']['avatar']['href']),
                'post_time'         => new xmlrpcval($topic['time'], 'dateTime.iso8601'),
                'timestamp'         => new xmlrpcval($topic['timestamp'], 'string'),
                'reply_number'      => new xmlrpcval($topic_info['num_replies'], 'int'),
    ...
    foreach($context['reports'] as $report)
    {
        ...
        $post_list[] = new xmlrpcval(array(
            ...
            'post_author_id'    => new xmlrpcval(basic_clean($report['author']['id']), 'string'),
             'post_author_user_type'  => new xmlrpcval(tt_get_user_type($report['author']['id']), 'string'),
           'icon_url'          => new xmlrpcval($avatar),
            'post_time'         => new xmlrpcval($report['time_started'], 'dateTime.iso8601'),
            'timestamp'         => new xmlrpcval($report['timestamp_started'], 'string'),
            'short_content'     => new xmlrpcval(basic_clean($report['body'], 200, 1), 'base64'),
...
{
    global $context, $modSettings, $txt, $tt_sso_result;


    if($_POST['action'] == 'register') return register_func();
    if($_POST['action'] == 'login2' &&  !$tt_sso_result->register) return login_func();

    if (!empty($tt_sso_result)){
        $errors = $tt_sso_result->errors;
    }else {
        $errors = array(isset($txt['registration_errors_occurred']) ? $txt['registration_errors_occurred'] : 'The following errors were detected in your registration.');
    }
    if (!empty($errors)){
        $result = array(
            'result' => new xmlrpcval(false, 'boolean'),
    ...
    $result = new xmlrpcval(array(
            ...
            'result_text'   => new xmlrpcval($result_text, 'base64')),
        'struct');
    if($tt_sso_result->register)
    {
        $result->me['struct']['register'] =  new xmlrpcval(true, 'boolean');
    }
    return new xmlrpcresp($result);
}
...
{
    global $context, $user_info, $user_profile, $settings, $scripturl, $modSettings, $smcFunc, $profile_fields, $txt;
    ...
    if (!empty($_REQUEST['u'])){
        ...
        $result['result'] = new xmlrpcval(TRUE, 'boolean');
        $result['user_id'] = new xmlrpcval($user_profile[$_REQUEST['u']]['id_member'], 'string');
        $result['login_name'] = new xmlrpcval(basic_clean($user_profile[$_REQUEST['u']]['member_name']), 'base64');
        $result['display_name'] = new xmlrpcval(basic_clean($user_profile[$_REQUEST['u']]['real_name']), 'base64');
        $result['avatar'] = new xmlrpcval($avatar, 'string');
    }
    ...
    if (isset($context['profile_fields']) && !empty($context['profile_fields'])){
        foreach ($context['profile_fields'] as $key => $profile_field){
            $field_type = tt_handle_custome_field_type($profile_field['type']);

            $description = '';
            if (isset($profile_field['subtext'])){
                $description = $profile_field['subtext'];
            }

            $field_option = '';
            if (isset($profile_field['options']) && !empty($profile_field['options'])){
                $options = $profile_field['options'];
                if (!is_array($options)){
                    $options = eval($options);
                }
                // Assuming we now have some!
                if (is_array($options)){
                    foreach ($options as $title => $text){
                        if (empty($text)) continue;
                        $field_option .= $title.'='.$text.'|';
                    }
                    $field_option = substr($field_option, 0, strlen($field_option)-1);
                }
            }

            $default = '';

            if (empty($field_option) && $custom_field['field_type'] == 'check'){
                $field_option = "0={$txt['no']}|1={$txt['yes']}";
            ...
            $custom_register_fields[] = new xmlrpcval( array(
                'name'        => new xmlrpcval($profile_field['label'], 'base64'),
                'description' => new xmlrpcval($description, 'base64'),
                'key'         => new xmlrpcval($key, 'string'),
                'type'        => new xmlrpcval($field_type, 'string'),
                'options'     => new xmlrpcval($field_option, 'base64'),
                'default'     => new xmlrpcval($default, 'base64'),
                'format'      => new xmlrpcval($format, 'base64'),
            ), 'struct');
        }
    }
    ...
    while ($custom_field = $smcFunc['db_fetch_assoc']($request)){
        $field_type = tt_handle_custome_field_type($custom_field['field_type']);

        $field_option = '';
        $default = '';
        if (!empty($custom_field['field_options'])){
            $options = explode(',', $custom_field['field_options']);
            foreach ($options as $title => $text){
                $field_option .= $title.'='.$text.'|';
                if (isset($custom_field['default_value']) && !empty($custom_field['default_value']) && $custom_field['default_value'] == $text){
                    $default .= $title.'|';
                }
            }
        ...
        $custom_register_fields[] = new xmlrpcval(array(
            'name'        => new xmlrpcval($custom_field['field_name'], 'base64'),
            'description' => new xmlrpcval($custom_field['field_desc'], 'base64'),
            'key'         => new xmlrpcval($custom_field['col_name'], 'string'),
            'type'        => new xmlrpcval($field_type, 'string'),
            'options'     => new xmlrpcval($field_option, 'base64'),
            'default'     => new xmlrpcval($default, 'base64'),
            'format'      => new xmlrpcval($format, 'base64'),
    ...
    if (isset($context['custom_fields']) && !empty($context['custom_fields'])){
        foreach ($context['custom_fields'] as $custom_field){

        }
    }

    if (!empty($custom_register_fields)){
        $result['custom_register_fields'] = new xmlrpcval($custom_register_fields, 'array');
    }
...
function activate_account_func(){
    $result = array();
    $result['result'] = new xmlrpcval(isset($_POST['activate_result']) && $_POST['activate_result'], 'boolean');
    if (isset($_POST['activate_status']) && !empty($_POST['activate_status'])){
        $result['status'] = new xmlrpcval($_POST['activate_status'], 'string');
    }

    if (isset($_POST['activate_result_text']) && !empty($_POST['activate_result_text'])){
        $result['result_text'] = new xmlrpcval($_POST['activate_result_text'], 'base64');
    }
...
function get_topic_participants_func(){
    ...
    $mobi_api_key = $modSettings['tp_push_key'];
    ...
    foreach ($user_profile as $user){
        if (isset($_POST['count_of_participants']) && $_POST['count_of_participants'] != -1 && count($xmlrpc_post) >= $_POST['count_of_participants']){
            break;
        }
        if (!empty($user['id_member']) && !array_key_exists($user['id_member'], $xmlrpc_post)){
            $xmlrpc_post[$user['id_member']] = array(
                'user_id'         => new xmlrpcval($user['id_member'], 'string'),
                'username'        => new xmlrpcval($user['real_name'], 'base64'),
                'icon_url'        => new xmlrpcval($user['avatar'], 'string'),
                'enc_email'       => new xmlrpcval(base64_encode(tt_encrypt(trim($user['email_address']), $mobi_api_key)), 'string'),
            );
            $user_list[] = new xmlrpcval($xmlrpc_post[$user['id_member']], 'struct');
        }
    }
    $result = array(
        'result' => new xmlrpcval(true, 'boolean'),
