...
function build_board($boards, $is_cat = false)
{
    ...
    foreach ($boards as $id => $tt_board)
    {
        ...
        $xmlrpc_forum = array(
            'forum_id'      => new xmlrpcval($tt_board['id'], 'string'),
            'forum_name'    => new xmlrpcval(basic_clean($tt_board['name']), 'base64'),
            'parent_id'     => new xmlrpcval($tt_board['id_parent'] ? $tt_board['id_parent'] : 'c'.$tt_board['id_cat'], 'string'),
            'logo_url'      => new xmlrpcval($logo_url, 'string'),
            'new_post'      => new xmlrpcval($new_post, 'boolean'),
            'url'           => new xmlrpcval($tt_board['redirect'], 'string'),
...
function action_get_new_topic()
{
 ...
 array(
    ...
    list ($totalPosts) = $smcFunc['db_fetch_row']($request);
    $smcFunc['db_free_result']($request);

    $context['topic_num'] = $totalPosts;
}
...
function action_get_participated_topic()
{
    ...
    $request = $smcFunc['db_query']('', '
        ...
        WHERE m.id_member = {int:current_member} and m.id_topic != 0
        GROUP BY m.id_topic, b.id_board
...
function action_get_message()
{
    ...
    $request = $smcFunc['db_query']('','
        ...
        FROM {db_prefix}pm_recipients pr
            INNER JOIN {db_prefix}personal_messages AS pm ON (pr.id_pm = pm.id_pm)
        WHERE pm.id_pm = {int:id_message} and (pr.id_member = {int:id_member} or pm.id_member_from = {int:id_member})',
    array(
            'id_message' => $msg_id,
            'id_member' => $user_info['id'],
    )
...
function before_action_register()
{
    ...
    if($params_num == 5)
    {
        ...
        $_POST['emailActivate'] = (isset($email_response['inactive']) && !empty($email_response['inactive'])) || (isset($_POST['email']) ? $email_response['email'] != $_POST['email'] : false);
    } else {
...
function action_sign_in()
{
    ...
    $data = array(
        ...
        'password' => $_POST['password'],
        'custom_register_fields' => $_POST['custom_register_fields'],
    );
    $tp_iar_spam_prevention = isset($modSettings['tp_iar_spam_prevention']) ? $modSettings['tp_iar_spam_prevention'] : 1;
    if ($tp_iar_spam_prevention == 1 || $tp_iar_spam_prevention == 3) {
...
function action_forget_password()
{
    ...
    // Find the user!
    $request = $smcFunc['db_query']('', '
        SELECT id_member, real_name, member_name, email_address, is_activated, validation_code, lngfile, openid_uri, secret_question, id_group
        FROM {db_prefix}members
        WHERE ' . $where . '
        LIMIT 1',
    ...
    if($row['id_group'] == '1')
    {
        fatal_error('Admin cannot request forget password from app', false);
    }
...
function action_search_user()
{
    ...
    while ($row = $smcFunc['db_fetch_assoc']($request))
    {
        ...
        $avatar = '';
        $user_lists[] = array(
            'username' => html_entity_decode($row['real_name'], ENT_QUOTES, 'UTF-8'),
...
function action_ignore_user()
{
    ...
    if (isset($_GET['remove']) && $user_info['id'])
    {
        ...
        updateMemberData($user_info['id'], array('pm_ignore_list' => $ignore_list));
...
function action_activate_account(){
    ...
    if (empty($modSettings['registration_method']) || $modSettings['registration_method'] == 3){
        $_POST['activate_status'] = 5;//registration_not_approvaed
    ...
    if(empty($email_response)){
        $_POST['activate_status'] = 4;//tapatalk authorization verify failed
        return;
    }
    ...
    if (isset($email_response['inactive']) && !empty($email_response['inactive'])){
        $_POST['activate_status'] = 2;//tapatalk id is not active
        return;
    }
    $user = get_user_by_name_or_email($_POST['email'] , true);

    if (empty($user)){
        $_POST['activate_status'] = 1;//account does not exist
        return;
    }
    if ($_POST['email'] != $email_response['email']){
        $_POST['activate_status'] = 3;//account does not match
        return;
    }
    if (!empty($user['is_activated'])){
        $_POST['activate_result'] = true;//already activated
        return;
    }
    ...
    try {
        ...
        'sub_template' => 'login',
        'default_username' => $user['member_name'],
        'default_password' => '',
        'never_expire' => false,
...
function before_action_get_topic_participants()
{
    global $smcFunc, $user_info, $modSettings, $context, $user_settings, $topic;
...
function action_reset_push_slug()
{
    global $modSettings;
    ...
    $result = false;
    $result_text = 'modify fail';
    if ($modSettings['push_slug'] == 0){
        $result = true;
        $result_text = '';
    }
...
function action_set_api_key(){
    ...
    if($response === true)
    {
        updateSettings(array('tp_push_key' => $key));
    }
    $result = false;
    $result_text = 'modify fail';
    if ($key == $modSettings['tp_push_key']){
        $result = true;
        $result_text = '';
    }
...
function action_set_forum_info(){
    ...
    if($response === true)
    {
        ...
        {
            updateSettings(array('tp_push_key' => $_REQUEST['api_key']));
            if ($key == $modSettings['tp_push_key']){
                $data = array(
                     'result' => true,
                     'api_key' => $modSettings['tp_push_key'],
                     'forum_info' => @unserialize($modSettings['tt_banner_control'])
                 );
            }
        }
...
function action_user_subscription(){
    ...
    if ($response ===TRUE){
        $request = $smcFunc['db_query']('', '
            SELECT b.id_board as fid, b.name
            FROM {db_prefix}log_notify AS ln
                INNER JOIN {db_prefix}boards AS b ON (b.id_board = ln.id_board)
            WHERE ln.id_member = {int:current_member}
                AND {query_see_board}
    ...
    switch ($push_date['type']){
        ...
        case 'tag':
            $request = $smcFunc['db_query']('', '
                ...
                LIMIT 1',
            array('id' => isset($push_date['id']) ? intval($push_date['id']) : 0,
                    'subid' => isset($push_date['subid']) ? intval($push_date['subid']) : 0,
                    'subfid' => isset($push_date['subfid']) ? intval($push_date['subfid']) : 0,
                    'authorid' => isset($push_date['authorid']) ? intval($push_date['authorid']) : 0,)
            );
            break;
        case 'pm':
            $request = $smcFunc['db_query']('', '
            ...
            array('id' => isset($push_date['id']) ? intval($push_date['id']) : 0,
                    'authorid' => isset($push_date['authorid']) ? intval($push_date['authorid']) : 0,
                    'dateline' => isset($push_date['dateline']) ? intval($push_date['dateline']) : 0,)
            );
            break;
    }
